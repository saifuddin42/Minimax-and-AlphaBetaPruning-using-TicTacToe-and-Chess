...
# Maximizer (X)
def maximizerMiniMax():
    global currentBoard
    global moveCounter 
    
    # initial worst value for maximizer
    bestMaxVal = -infinity
    ...
    for i in range(0, 3):
        for j in range(0, 3):
            ...
            if currentBoard[i][j] == ' ':
                currentBoard[i][j] = 'X'
                # call the minimizer
                (val, minI, minJ) = minimizerMiniMax()
    ...
    for i in range(0, 3):
        for j in range(0, 3):
            ...
            if currentBoard[i][j] == ' ':
                currentBoard[i][j] = 'O'
                # call the maximizer
                (val, maxI, maxJ) = maximizerMiniMax()
                # get the best value and index
                if val < bestMinVal:
                    bestMinVal = val
                    boardI = i
...
def playMiniMax():
    ...
    while True:
        ...
        if currentTurn == 'X':
            ...
            start = time.time()
            (val, boardI, boardJ) = maximizerMiniMax()
            end = time.time()
        ...
        # call the minimizer in O's turn and update the board with the best returned value
            ...
            moveCounter = 0
            start = time.time()
            (val, boardI, boardJ) = minimizerMiniMax()
            end = time.time()
            print('Time Taken: {}s'.format(end - start))
            print('Best Move i,j = {},{}'.format(boardI, boardJ))
            print("Positions Evaluated:", moveCounter)
